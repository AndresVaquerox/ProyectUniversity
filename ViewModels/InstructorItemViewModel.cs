...
namespace UniversityApp.ViewModels
{
    ...
    public class InstructorItemViewModel : InstructorDTO
    {
        ...
        async Task DeleteInstructor()
        {
            ...
            var instructortViewModel = InstructorViewModel.GetInstance();
            var instructorDeleted = instructortViewModel.AllInstructors.FirstOrDefault(x => x.ID == this.ID);
            instructortViewModel.AllInstructors.Remove(instructorDeleted);
            instructortViewModel.GetInstructorsByName();
